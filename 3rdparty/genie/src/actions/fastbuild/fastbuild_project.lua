...
local function binary(prj, cfg, useconfig, bintype, commonbasepath)
	...
	local linklibs = premake.getlinks(cfg, "system", "fullpath")
...
local function customtasks(prj)
	local files = { }
	...
	if #files > 0  then
		...
		for name, custombuildtask in iter.sortByKeys(groupedBuildTasks or {}) do
			local name = path.getrelative(prj.location,name)
			local args = "/C "
			local input = ""
			local output = ""
			for _, buildtask in ipairs(custombuildtask or {}) do
				for _, cmdline in ipairs(buildtask[4] or {}) do
					...
					for _, depdata in ipairs(buildtask[3] or {}) do
						args = string.gsub(args,"%$%(" .. num .."%)", string.format("%s ",path.getrelative(prj.location,depdata)))
						num = num + 1
					end
					args = string.gsub(args, "%$%(<%)", path.getrelative(prj.location, buildtask[1]))
					args = string.gsub(args, "%$%(@%)", path.getrelative(prj.location, buildtask[2]))
					args = args .. " & "
				end
				input = path.getrelative(prj.location,buildtask[1])
				output = path.getrelative(prj.location,buildtask[2])
			end
			_p(1,'Exec() {')
			_p(2,'.ExecExecutable = \'%s\'', os.getenv("COMSPEC"))
			_p(2,'.ExecArguments = \'%s\'', args:sub(0,-3))
			_p(2,'.ExecInput = \'%s\'', input)
			_p(2,'.ExecOutput = \'%s\'', output)
			_p(1,'}')
		end
	end
end
	...
	for _, dependency in ipairs(prj.dependency or {}) do
		for _, dep in ipairs(dependency or {}) do
			if (dep[3]==nil or dep[3]==false) then
				table.insert(deps, path.getrelative(prj.location, dep[2]))
			end
		end
	end
	table.sort(deps)
	if #deps > 0  then
		_p(1, ".PreBuildDependencies = {")
